...
shinyServer(function(input, output) 
  ...
  output$populationImpactByState <- renderPlot(
    ...
    p <- p + geom_map(aes(fill = Affected), map = states_map, colour='white') + expand_limits(x = states_map$long, y = states_map$lat)
  ...
  output$economicImpactByState <- renderPlot({
    ...
    p <- ggplot(data, aes(map_id = STATE))
    p <- p + geom_map(aes(fill = Damages), map = states_map, colour='white') + expand_limits(x = states_map$long, y = states_map$lat)
    p <- p + coord_map() + theme_bw()
    p <- p + labs(x = "Long", y = "Lat", title = title)
    print(p)
  })
...
output$downloadData <- downloadHandler(filename = 'storm_data.csv',
